"""
Training Strategy for PatchHRNet - Orchestrator for different training approaches.

RECEIVES ALL DATA FROM training_block_1 - No imports from training_block_1!

RESPONSIBILITIES:
- Implement strategy pattern (split vs LOSO) for PatchHRNet
...
training_block_1.py (Entry point - prepares all data)
  → training_strategy_patch.py (Receives all data as parameters)
...
from src.models.architecture.hr_patch import PatchHRNet
...
# ==================== TRAINING STRATEGY CLASS FOR PATCHHRNET ====================

class TrainingStrategyPatch:
    """
    Implements Strategy pattern for training method selection with PatchHRNet.
    ...
    Uses PatchHRNet (Transformer-based) model architecture.
    """
    ...
    def _train_split(self) -> None:
        ...
        print("="*70)
        print("TRAINING WITH FIXED SPLIT METHOD (PatchHRNet)")
        print("="*70 + "\n")
        ...
        device, model, optimizer, loss_function, scheduler_config, model_run_dir = \
        ...
        _, _, _, epochs_data = \
            self._run_training_loop(model, train_loader, valid_loader, optimizer,
                                  loss_function, device, scheduler_config, model_run_dir)
        print()
    ...
    def _train_loso(self) -> None:
        ...
        print("="*70)
        print("LOSO WITH OPTUNA HYPERPARAMETER TUNING (PatchHRNet)")
        print("="*70 + "\n")
    ...
    def _train_loso_full(self) -> List[Dict]:
        ...
        for fold_idx, test_subject in enumerate(all_subjects, 1):
            ...
            print("Initializing training components...")
            device, model, optimizer, loss_function, scheduler_config, _ = \
            ...
            _, _, _, epochs_data = \
                self._run_training_loop(model, train_loader, valid_loader, optimizer,
                                      loss_function, device, scheduler_config, models_fold_dir)
            print()
    ...
    def _tune_hyperparameters_with_optuna(self) -> Dict:
        ...
        def objective(trial: optuna.Trial) -> float:
            ...
            for _, test_subject in enumerate(mini_subjects, 1):
                ...
                patch_masking_enabled = self.config.get('patch_masking', True)
                mask_ratio = self.config.get('patch_mask_ratio', 0.1)

                for epoch in range(tuning_epochs):
                    # Training with patch masking
                    model.train()
                    for x_batch, y_batch in train_loader:
                        ...
                        # Apply patch masking
                        if patch_masking_enabled:
                            x_batch = self.helper.apply_patch_masking(x_batch, mask_ratio=mask_ratio)
    ...
    def _initialize_loss_and_scheduler(self, optimizer) -> Tuple:
        """Initialize loss function and warmup + cosine scheduler for Transformer.
        ...
        Returns a tuple with loss function and a dict containing warmup/cosine schedulers.
        """
        ...
        # For Transformer, we use warmup + cosine instead of ReduceLROnPlateau
        # Store scheduler config in a dict - will be instantiated later when we know num_batches
        scheduler_dict = {
            'type': 'warmup_cosine',
        ...
        return loss_function, scheduler_dict

    def _initialize_training_components(self) -> Tuple:
        """Initialize model, optimizer, and scheduler config for Transformer."""
        device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
        print(f"✓ Using device: {device}\n")

        model = PatchHRNet().to(device)
        optimizer = self._initialize_optimizer(model)
        loss_function, scheduler_config = self._initialize_loss_and_scheduler(optimizer)

        model_run_dir = os.path.join(f"../../../models/block_1/{self.version}",
                                    os.path.basename(self.run_dir or ""))
        os.makedirs(model_run_dir, exist_ok=True)

        return device, model, optimizer, loss_function, scheduler_config, model_run_dir

    def _initialize_training_components_for_fold(self, models_fold_dir: str) -> Tuple:
        """Initialize model, optimizer, and scheduler config for a fold."""
        device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
        ...
        return device, model, optimizer, loss_function, scheduler_config, models_fold_dir

    def _run_training_loop(self, model, train_loader, valid_loader, optimizer, loss_function,
                           device, scheduler_config, num_epochs_or_model_dir) -> Tuple:
        """Execute training and validation loop with warmup + cosine scheduler for Transformers.

        Handles:
        - Warmup + Cosine annealing for stable optimization
        - Patch masking for regularization
        ...
        # Initialize warmup + cosine schedulers
        ...
        print("="*70)
        print("STARTING TRAINING (Transformer with Warmup + Cosine)")
        print("="*70 + "\n")
        ...
        for epoch in range(num_epochs):
            ...
            for batch_idx, (x_batch, y_batch) in enumerate(train_loader):
                ...
                # Apply GPU-Native patch masking (make sure you use the PyTorch version!)
                if patch_masking_enabled:
                    x_batch = self.helper.apply_patch_masking(x_batch, mask_ratio=mask_ratio)
                ...
                # --- FIX 1: Cost-sensitive loss (Assuming loss_function has reduction='none') ---
                base_loss_tensor = loss_function(pred, target)  # Returns shape (Batch,)
                weights = torch.ones_like(target)
                weights[target > 120.0] = 1.5
                weighted_loss = (base_loss_tensor * weights).mean()  # Now we reduce it!
                ...
                # --- FIX 3: Single Scheduler Step ---
                # Assuming you combined them into a SequentialLR object named 'scheduler'
                scheduler.step()
                ...
                if (batch_idx + 1) % 10 == 0:
                    ...
                    print(f"  Batch [{batch_idx + 1}/{len(train_loader)}] - "
                          f"Loss: {weighted_loss.item():.4f} - LR: {current_lr:.6f}")
    ...
    def _create_ensemble_model(self, models_run_dir: str, num_folds: int) -> None:
        ...
        averaged_model = PatchHRNet().to(device)
    ...
    def _save_best_hyperparameters(self, best_params: Dict, avg_mae: float,
        ...
        hyperparams_data = {
            'source': 'Optuna Hyperparameter Tuning (PatchHRNet)',
            'hyperparameters': {
